					...
					<form action=" " method="post" enctype="multipart/form-data">
						<label>Titulo:</label><br/> 
							<div class="input-group">
								...
								<input type="text" class="form-control" name="titulo" maxlength="200" required/><br />
							</div>
						<label>Autor:</label><br/><div class="input-group"><span class="input-group-addon"><i class="fa  fa-group"></i></span><input class="form-control" type="text" name="autor" value = "<?php echo $dados['completo']; ?>"/><br/></div>
					   
						<label>Imagem:</label><br/><input type="file" name="arquivo" required/><br/>

						<label>Resumo:</label><br/>
						<textarea class="textarea" id="resumo" name="resumo" maxlength="600" placeholder="Pequeno Resumo" style="   resize: none; width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #CCC; padding: 10px;" required></textarea></br>
						<label>Noticia:</label> <br/> 
						<textarea cols="80" id="texto" name="texto" rows="10" required> </textarea><br/>
					  <br/>
								...
								if(isset($_POST['noticia'])){
									...
									if (array_search($file_extension, $_UP['extensoes']) === false) {
										...
										if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
											$sql = "INSERT  INTO `news` (titulo, texto, autor, resumo, imagem) 
											VALUES ('$titulo', '$texto', '$autor', '$resumo', '$nome_final')";
									 
											if ($mysqli->query($sql)) {
												echo "<script>alert('Noticia adicionada com sucesso'); location.href='index.php';</script>";
											} else {
