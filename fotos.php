         ...
         <div class="col-sm-9 blog-main">
          ...
          <div class="blog-post">
			<?php
				...
				$sql = mysqli_query($con, "SELECT * FROM img ORDER by data DESC LIMIT $inicio, $quantidade");
			...
			<section id="paginanoticia">
				<span><?php echo $res['nome']."</a>"; ?></span>
				<h5><?php echo '' . date('d', strtotime($res['data'])). " de " .$mes[date('m', strtotime($res['data']))]. " de ". date('Y', strtotime($res['data'])). " ás ". date('H:i', strtotime($res['data']));?></h5>
				<?php echo "<a href = 'painel/restrito/galeria/".$res['image']."'>"?>
				<img alt="" src="painel/restrito/galeria/<?php echo $res['image']; ?>" height="350px" width="100%">			</a>
			</section>
			<?php }} ?>
	<section class = "pagination">

       	<?php
         $sql2 = mysqli_query($con, "SELECT * FROM img");
         $total = mysqli_num_rows($sql2);
         $paginas = ceil($total / $quantidade);
         $links = 5;
         echo "<li><a href = '?pg=1' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a></li>";
         for($i = $pg-$links; $i <= $pg-1; $i++) {
         	if($i > 0) {
         		echo "<li><a href = '?pg=".$i."' class='numeros'>" .$i. "</a></li>"."";
         	}
         }
         echo "<li><a href=#>$pg</a></li>"."";
         for($i = $pg+1; $i <= $pg+$links; $i++) {
         	if($i < $paginas) {
         		echo "<li><a href = '?pg=".$i."'>" .$i. "</a></li>"."";
         	}
         }
          echo "<li><a href='?pg=".$paginas."' aria-label='Next'> <span aria-hidden='true'>&raquo;</span></a></li>";
		?>
    </section>
          </div><!-- /.blog-post -->
